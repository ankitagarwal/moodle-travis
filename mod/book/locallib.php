        ...
        foreach ($chapters as $ch) {
            ...
            if ($i != 1) {
                $toc .= html_writer::link(new moodle_url('move.php', array('id' => $cm->id, 'chapterid' => $ch->id, 'up' => '1', 'sesskey' => $USER->sesskey)),
                        $OUTPUT->pix_icon('t/down', get_string('moveupchapter', 'mod_book', $title)),
                        array('title' => get_string('moveupchapter', 'mod_book', $title)));
            }
            if ($i != count($chapters)) {
                $toc .= html_writer::link(new moodle_url('move.php', array('id' => $cm->id, 'chapterid' => $ch->id, 'up' => '0', 'sesskey' => $USER->sesskey)),
                        $OUTPUT->pix_icon('t/down', get_string('movedownchapter', 'mod_book', $title)),
                        array('title' => get_string('movedownchapter', 'mod_book', $title)));
            }
