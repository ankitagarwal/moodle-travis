...
class core_files_external extends external_api {
        ...
        return new external_function_parameters(
            array(
                ...
                'contextlevel' => new external_value(PARAM_ALPHA, 'The context level to put the file in,
                        (block, course, coursecat, system, user, module)', VALUE_DEAFULT, null),
                'instanceid' => new external_value(PARAM_INT, 'The Instance id of item associated with the context level', VALUE_OPTIONAL, null)
            )
        );
    }
    ...
    public static function upload($contextid, $component, $filearea, $itemid, $filepath, $filename, $filecontent, $contextlevel = null, $instanceid = null) {
        ...
        // We need to preserve backword compatibility. Context id is no more a requird.
         if (empty($fileinfo['contextid'])) {
