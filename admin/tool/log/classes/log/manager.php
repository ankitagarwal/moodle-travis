...
class manager implements \core\log\manager {
     ...
     */
    public function get_supported_reports($store) {

        $allstores = self::get_store_plugins();
        if (empty($allstores[$store])) {
            // Store doesn't exist.
            return array();
        }
        ...
        foreach ($reports as $report => $fulldir) {
            ...
            if (file_exists($file)) {
                require_once($file);
                $function = 'report_' . $report . '_supports_store';
    ...
    public function get_supported_stores($report) {
        ...
        $function = 'report_' . $report . '_supports_store';
        $file = $CFG->dirroot . '/report/' . $report .'/lib.php';
