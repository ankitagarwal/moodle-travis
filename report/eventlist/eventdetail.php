...
$completelist = \report_eventlist\list_generator::get_all_events_list(false);
...
$directory = $directory . '/' . $filename . '.php';
$title = $component[1] . ' ' . $filename;

// Define event information.
$eventinformation = array('title' => $title);
$eventcontents = file_get_contents($directory);
...
if (!$ref->isAbstract()) {
    ...
    if ($eventname != '\core\event\base') {
        ...
        if (!empty($result[2])) {
            $eventinformation['crud'] = \report_eventlist\list_generator::get_crud_string($result[2]);
        }
        ...
        if (!empty($result[2])) {
            ...
            $eventinformation['edulevel'] = \report_eventlist\list_generator::get_edulevel_string($educationlevel);
        }
...
$allobserverslist = \report_eventlist\list_generator::get_observer_list();
$observers = array();
if (isset($allobserverslist[$eventname])) {
    $observers = $allobserverslist[$eventname];
}
